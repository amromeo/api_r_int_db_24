...
We can use this function to bring in the information related to the lab orders to decode the item IDs.
...
2.  Join LABEVENTS to D_LABITEMS table
...
If you have time:
How would you change the analytic strategy to correctly answer the question?
...
# Access tables
diagnoses_icd9 <- tbl(mimic, "d_icd_diagnoses") %>% filter(icd_version == 9) %>% collect()
diagnoses_icd10 <- tbl(mimic, "d_icd_diagnoses") %>% filter(icd_version == 10) %>% collect()

# Perform a fuzzy join using string distance on descriptions
result <- diagnoses_icd9 %>%
  stringdist_left_join(diagnoses_icd10, by = c("long_title" = "long_title"), method = "jw", max_dist = 0.2)
