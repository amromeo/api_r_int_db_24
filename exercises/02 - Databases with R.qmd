...
```{r}
glucose_items <- filter(d_labitems, label == "Glucose")
```
...
Using `count()` generate a new table object tests_per_subject that shows the number of lab events per subject_id.
