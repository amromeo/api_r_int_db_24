...
Run the setup chunk below to load the packages we need for a database connection. The DBI package provides functions to connect to and work with various types of databases. The RSQLite contains a specific driver to allow connections with SQLite databases, which are stored a single file. We are also loading the connections package that we will explain later in the lesson.
...
We mentioned previously that connection and table objects do not have the same characteristics as typical data frames, for which the contents are available in your environment. This is an advantage of lazy evaluation and can support the preservation of your system's memory. However, it is possible to bring the contents of a table into memory as a data frame using the `collect()` function.
