...
The following code chunks will use the **REDCapR** package to connect to a REDCap database and download its data into an object called `my_redcap_data`. Run the code chunks.
...
2.  How many rows are in the `adverse_events` table? **53**

3.  Is the `medical_history` instrument repeating or nonrepeating? How can you tell? Why would it be designed this way? **repeating - allows capturing multiple medical diagnoses per patient**
...
1.  What is the average age of study participants? Standard deviation? **Average is 8.1 years, SD is 6.6 years.**
2.  In the `hematology` table, which blood counts are approximately normally distributed? Which ones aren't? RBC, HgB, Hct, and Plt are all approximately normally distributed. The other ones aren't. See the code below to extract the skimr-generated numeric histograms.

```{r}
### remove from exercises

redcart[[4]][[11]] |>   # Pull out the metadata table for demographics
  dplyr::filter(skim_type == "numeric") |> # only include numeric fields
  dplyr::select(field_name, numeric.hist)
```
...
### solution below
redcart |>
  write_redcap_xlsx("redcart.xlsx")
```
